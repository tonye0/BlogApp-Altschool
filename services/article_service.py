...
class ArticleService:
    ...
    def create(self, req, db: Session):
        new_article = models.Article(
            title=req.title, article_content=req.article_content, author_username=req.author_username)

        username = db.query(models.User).filter(
            (models.User.username == req.author_username)).first()
    ...
    def update(self, id: int, req, db: Session):
        ...
        article_update = db.query(models.Article).filter(
    ...
    def delete(self, id: int, db: Session):
        article_delete = db.query(models.Article).filter(
            models.Article.id == id)
    ...
    def get_article(self, id: int, db: Session):
        author = db.query(models.Article).filter(
            models.Article.id == id).first()
        if not author:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Article not found")
        return author
